 ...
 * @name NoteName
 * @author Scalz
 * @description Shows note in parentheses after name.
 * @version 0.0.1
 * @invite inviteCode
 * @authorId 672036959212011571
 * @source https://github.com/mikkel-ol/BetterDiscordAddons
 */
 ...
 module.exports = (_ => {
	...
	return !window.BDFDB_Global || (!window.BDFDB_Global.loaded && !window.BDFDB_Global.started) ? class {
		...
		return class NoteName extends Plugin {
			onLoad () {
				this.defaults = {
					settings: {
						replaceOwn:				{value: false, 	inner: false,		description: "Replace your own name"},
						replaceBots:			{value: true, 	inner: false,		description: "Replace the name of bots"},
			...
			getNewName (user) {
				...
				// ! This is undefined until note has been manually loaded
				// TODO: Needs to be loaded automatically somehow..
				let note = BdApi.findModuleByProps('getNote').getNote(user.id)?.note;

				if (user.id == BDFDB.UserUtils.me.id && !settings.replaceOwn || user.bot && !settings.replaceBots) return username != origUser.username ? username : (member.nick || username);
				return (note && note.length > 0) ? (member.nick || username) + " (" + note + ")" : undefined;
			}
		};
